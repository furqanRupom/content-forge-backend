...
class Service {
  ...
  async getAllGenerations(query: Record<string, unknown>, user: IRequestUser) {
    const baseQuery = {
      ...query,
      userId: user.userId,
    };

    const generationQueryInstance = new QueryBuilder(prisma.generationJob, baseQuery as IQueryParams, {
      searchableFields: ["inputPrompt", "errorMessage", "model"],
      filterableFields: ["templateId", "status", "userId", "model"],
    });

    const result = await generationQueryInstance
      .search()
      .filter()
      .paginate()
      .sort()
